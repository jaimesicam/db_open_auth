...
static char *opt_db_open_auth_oidc_token_file = nullptr;
static char *opt_db_open_auth_oidc_token = nullptr;
...
#include "authentication_kerberos_clientopt-longopts.h"
    {"db-open-auth-oidc-token-file", OPT_DB_OPEN_AUTH_OIDC_TOKEN_FILE,
     "Path to file containing OIDC ID token for db_open_auth_oidc_client.",
     &opt_db_open_auth_oidc_token_file, &opt_db_open_auth_oidc_token_file, nullptr,
     GET_STR, OPT_ARG, 0, 0, 0, nullptr, 0, nullptr},
    {"db-open-auth-oidc-token", OPT_DB_OPEN_AUTH_OIDC_TOKEN,
     "OIDC ID token value for db_open_auth_oidc_client.",
     &opt_db_open_auth_oidc_token, &opt_db_open_auth_oidc_token, nullptr, GET_STR,
...
static bool init_connection_options(MYSQL *mysql) {
  ...
  struct st_mysql_client_plugin *db_open_auth_oidc_pl = mysql_client_find_plugin(
      mysql, "db_open_auth_oidc_client", MYSQL_CLIENT_AUTHENTICATION_PLUGIN);
  if (opt_db_open_auth_oidc_token_file != nullptr) {
    if (!db_open_auth_oidc_pl) {
      put_info("Cannot load the db_open_auth_oidc_client plugin.", INFO_ERROR);
      return true;
    }
    if (mysql_plugin_options(db_open_auth_oidc_pl, "db_open_auth_oidc_token_file",
                             opt_db_open_auth_oidc_token_file)) {
      put_info("Failed to set db_open_auth_oidc_token_file for "
               "db_open_auth_oidc_client.",
               INFO_ERROR);
      return true;
    }
  }
  if (opt_db_open_auth_oidc_token != nullptr) {
    if (!db_open_auth_oidc_pl) {
      put_info("Cannot load the db_open_auth_oidc_client plugin.", INFO_ERROR);
      return true;
    }
    if (mysql_plugin_options(db_open_auth_oidc_pl, "db_open_auth_oidc_token",
                             opt_db_open_auth_oidc_token)) {
      put_info(
          "Failed to set db_open_auth_oidc_token for db_open_auth_oidc_client.",
          INFO_ERROR);
      return true;
    }
  }
